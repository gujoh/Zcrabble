...
class UnsafeBoardMutations {
    ...
    static void tilt3PiHalf(Board board){
        ...
        for (int i = 0; i <board.getBoardCells().length ; i++) {
            for (int j = 0; j <board.getBoardCells()[0].length ; j++) {
                ...
                tempBoard.getBoardCells()[j][board.getBoardCells().length-i-1] = new Cell(board.getCellWordMultiplier(i,j),newCell.GetCellLetterMultiplier(),newCell.getPlacedTile());
            }
        }
    ...
    static void tiltPiHalf(Board board){
        ...
        for (int i = 0; i <board.getBoardCells().length ; i++) {
            for (int j = 0; j <board.getBoardCells()[0].length ; j++) {
                Cell newCell = board.getBoardCells()[i][j];
                tempBoard.getBoardCells()[board.getBoardCells()[0].length-j-1][i] = new Cell(board.getCellWordMultiplier(i,j),newCell.GetCellLetterMultiplier(),newCell.getPlacedTile());
            }
        }
        board.copyBoardCells(tempBoard, true);
    }
    ...
    static void mirrorAroundCol7(Board board){
        ...
        for (int i = 0; i <board.getBoardCells().length ; i++) {
            for (int j = 0; j <board.getBoardCells()[0].length ; j++) {
                Cell newCell = board.getBoardCells()[i][j];
                tempBoard.getBoardCells()[board.getSize()-1-i][j] = new Cell(board.getCellWordMultiplier(i,j),newCell.GetCellLetterMultiplier(),newCell.getPlacedTile());
            }
        }
        board.copyBoardCells(tempBoard,true);
    }
